...
public class MultipleObjectsMake : _ObjectsMakeBase
{
    ...
    void Start()
    {
        m_Time = m_Time2 = Time.unscaledTime;
    ...
    void Update()
    {
        if (Time.unscaledTime > m_Time + m_startDelay)
        {
            if (Time.unscaledTime > m_Time2 + m_makeDelay && m_count < m_makeCount)
            {
                ...
                for (int i = 0; i < m_makeObjs.Length; i++)
                {
                    ...
                    ForceUnscaledTimeOnParticles(m_obj);
                }

                m_Time2 = Time.unscaledTime;
                m_count++;
            }
        }
    }

    private void ForceUnscaledTimeOnParticles(GameObject targetObj)
    {
        ParticleSystem[] particles = targetObj.GetComponentsInChildren<ParticleSystem>();
